...
Each action's hotkey SHALL be registered as a system-wide global hotkey that works while any application is focused.
...
The 3 default actions SHALL ship with pre-assigned hotkeys: Rewrite for Clarity (Control+Option+1), Grammar Fix (Control+Option+2), Paraphrase (Control+Option+3).
